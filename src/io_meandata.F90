...
!DS
CASE ('t050      ')
    call def_stream(nod2D, myDim_nod2D, 't050',     'temperature 50m',                'C',      tracers%data(1)%values(7,1:myDim_nod2D),  io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, partit, mesh)
CASE ('t100      ')
    call def_stream(nod2D, myDim_nod2D, 't100',     'temperature 100m',               'C',      tracers%data(1)%values(12,1:myDim_nod2D), io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, partit, mesh)
CASE ('s050      ')
    call def_stream(nod2D, myDim_nod2D, 's050',      'salinity 50m',                'psu',      tracers%data(2)%values(7,1:myDim_nod2D),  io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, partit, mesh)
CASE ('s100      ')
    call def_stream(nod2D, myDim_nod2D, 's100',      'salinity 100m',               'psu',      tracers%data(2)%values(12,1:myDim_nod2D), io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, partit, mesh)
CASE ('u050      ')
    call def_stream(elem2D, myDim_elem2D,  'u050',   'horizontal velocity 50m',     'm/s',      dynamics%uv(1,7,1:myDim_elem2D),          io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, partit, mesh)
CASE ('u100      ')
    call def_stream(elem2D, myDim_elem2D,  'u100',   'horizontal velocity 50m',     'm/s',      dynamics%uv(1,12,1:myDim_elem2D),         io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, partit, mesh)
CASE ('v050      ')
    call def_stream(elem2D, myDim_elem2D,  'v050',   'meridional velocity 50m',     'm/s',      dynamics%uv(2,7,1:myDim_elem2D),          io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, partit, mesh)
CASE ('v100      ')
    call def_stream(elem2D, myDim_elem2D,  'v100',   'meridional velocity 50m',     'm/s',      dynamics%uv(2,12,1:myDim_elem2D),         io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, partit, mesh)
!DS
CASE ('vve_5     ')
...
CASE ('unod30      ')
    call def_stream(nod2D, myDim_nod2D, 'unod30',   'zonal velocity at nodes', 'm/s', dynamics%uvnode(1,5,1:myDim_nod2D), io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, partit, mesh)
CASE ('vnod30      ')
    call def_stream(nod2D, myDim_nod2D, 'vnod30',   'meridional velocity at nodes', 'm/s', dynamics%uvnode(2,5,1:myDim_nod2D), io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, partit, mesh)   
CASE ('u30       ')
    call def_stream(elem2D, myDim_elem2D, 'u30',    'zonal velocity 30m','m/s',           dynamics%uv(1,5,1:myDim_elem2D),     io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, partit, mesh)
CASE ('unod100      ')
    call def_stream(nod2D, myDim_nod2D, 'unod30',   'zonal velocity at nodes', 'm/s', dynamics%uvnode(1,12,1:myDim_nod2D), io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, partit, mesh)
CASE ('vnod100      ')
    call def_stream(nod2D, myDim_nod2D, 'vnod30',   'meridional velocity at nodes', 'm/s', dynamics%uvnode(2,12,1:myDim_nod2D), io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, partit, mesh)   
CASE ('v30       ')
    call def_stream(elem2D, myDim_elem2D, 'v30',    'meridional velocity 30m','m/s',      dynamics%uv(2,5,1:myDim_elem2D),     io_list(i)%freq, io_list(i)%unit, io_list(i)%precision, partit, mesh)
